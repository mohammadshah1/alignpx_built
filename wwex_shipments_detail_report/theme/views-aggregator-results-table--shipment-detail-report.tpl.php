      ...
      <?php if (!empty($header)) : ?>
        <tr>
          <?php foreach ($header as $field => $label): ?>
            <?php 
						if ( (!user_access('access wwex reports') && ($field == 'field_shipment_booking_fee_2' || $field == 'field_carrier_rate_2' ) )){ ?>
    ...
    <?php foreach ($rows as $r => $row): ?>
      <tr <?php if (!empty($row_classes[$r])): ?>class="<?php print implode(' ', $row_classes[$r]); ?>"<?php endif ?>>
        <?php foreach ($row as $field => $content): ?>
				...
				if ( ($access == FALSE && ($field == 'field_shipment_booking_fee_2' || $field == 'field_carrier_rate_2' ) )) {
				?>
